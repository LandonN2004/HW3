...
def MakeDiagDom(A):
    """
    Reorder rows to attempt diagonal dominance by picking, for each column i,
    a remaining row r with the largest abs value in column i, and swapping it into row i.

    Works for BOTH square A and augmented [A|b] (N x (N+1)).
    """
    AA = dc(A)
    n = len(AA)
    mcols = len(AA[0])

    # We only care about the first n columns for the diagonal (ignore last b-column if augmented)
    for i in range(n):
        # Find best pivot row among i..n-1 based on abs value at column i
        best_row = i
        best_val = abs(AA[i][i])
